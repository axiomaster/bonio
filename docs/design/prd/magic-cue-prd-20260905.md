...
2. 胶囊弹出方向随 avatar 位置正确翻转（左/右各验证一次）。
3. 点击胶囊后文本出现在聊天输入框并自动发送（主路径真机验证；降级路径可演示）。
4. 无询问场景不弹胶囊，双击记忆功能回归正常。
5. 权限未授/开关关闭时给出明确引导，无崩溃无静默失败。
6. 胶囊 12s 自动收起，不遮挡 avatar，不影响拖动 avatar。
