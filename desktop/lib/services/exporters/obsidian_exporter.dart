  ...
  Future<ExportResult> exportNote(
    ...
    try {
      ...
      if (previousPath != null) {
        // Re-export: overwrite the existing file
        mdFile = File(previousPath);
        // Delete old file if renaming is needed (title changed)
        if (await mdFile.exists() && p.basenameWithoutExtension(mdFile.path) != safeTitle) {
          await mdFile.delete();
          mdFile = File(p.join(targetDir.path, '$safeTitle.md'));
        } else if (!await mdFile.exists()) {
          mdFile = File(p.join(targetDir.path, '$safeTitle.md'));
        }
      } else {
        // New export: find unique filename
      ...
      return ExportResult.success(
        message: 'Saved to $subFolder/${p.basename(mdFile.path)}',
