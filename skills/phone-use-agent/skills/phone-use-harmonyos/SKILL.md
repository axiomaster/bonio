...
## HarmonyOS vs Android Differences

| 功能 | HarmonyOS | Android |
|------|-----------|---------|
| 截图命令 | `snapshot_display` | `screencap -p` |
| 点击命令 | `/bin/uitest uiInput click` | `input tap` |
| 滑动命令 | `/bin/uitest uiInput swipe` | `input swipe` |
| 启动应用 | `aa start -a -b` | `am start -n` |
| 返回键 | `Back` | `KEYCODE_BACK` |
| Home键 | `Home` | `KEYCODE_HOME` |
| 部署路径 | `/data/local/bin/` | `/data/local/tmp/` |
| 连接工具 | HDC | ADB |
