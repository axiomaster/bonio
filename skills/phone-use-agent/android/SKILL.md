...
## Android vs HarmonyOS Differences

| 功能 | Android | HarmonyOS |
|------|---------|-----------|
| 截图命令 | `screencap -p` | `snapshot_display` |
| 点击命令 | `input tap` | `/bin/uitest uiInput click` |
| 滑动命令 | `input swipe` | `/bin/uitest uiInput swipe` |
| 启动应用 | `am start -n` | `aa start -a -b` |
| 返回键 | `KEYCODE_BACK` | `Back` |
| Home键 | `KEYCODE_HOME` | `Home` |
| 部署路径 | `/data/local/tmp/` | `/data/local/bin/` |
| 连接工具 | ADB | HDC |
