...
export function cleanContactQuery(query: string): string {
  let s = (query || '').trim()
...
export async function searchContacts(query: string, limit = 5): Promise<ContactRecord[]> {
  await getSqlite();
  ...
  try {
    ...
    const rows = stmt.all(pattern, pattern, pattern, needle, needle, prefix, prefix, maxLimit) as Array<Record<string, any>>;

    return rows.map((r) => {
      const phones = typeof r.phones === 'string' && r.phones ? r.phones.split(',').map((p: string) => p.trim()).filter(Boolean) : [];
      const emails = typeof r.emails === 'string' && r.emails ? r.emails.split(',').map((e: string) => e.trim()).filter(Boolean) : [];
      const item: ContactRecord = {
        id: Number(r.id),
        name: String(r.display_name || '未命名'),
        phones,
      };
      if (r.company) item.company = String(r.company);
      if (r.position) item.position = String(r.position);
      if (r.nickname) item.nickname = String(r.nickname);
      if (emails.length > 0) item.emails = emails;
      return item;
    });
  } catch (e) {
