...
import { IlinkHttpClient, type IlinkInboundMessage } from './ilink.js';
...
export class WechatChannel {
  private readonly driver: RunChatDriver;
  private ilink: IlinkHttpClient | null = null;
  private activeToken = '';
  ...
  async syncFromConfig(): Promise<void> {
    try {
      ...
      if (this.ilink && this.activeToken === binding.token && this.activeBaseUrl === binding.baseUrl) return;
      this.stop();
      this.allowFrom = binding.allowFrom;
      this.ilink = new IlinkHttpClient(
        binding.token,
        binding.baseUrl,
        bonioHome() + '/.bonio/wechat-state',
        (msg) => { void this.handleInbound(msg); },
        (m) => console.log(m),
      );
      this.ilink.start();
  ...
  private async handleInbound(msg: IlinkInboundMessage): Promise<void> {
    const text = msg.content.trim();
    if (!text || !msg.fromUserId) return;
    if (this.allowFrom.length > 0 && !this.allowFrom.includes(msg.fromUserId) && !this.allowFrom.includes('*')) return;
    if (msg.messageId) {
      const now = Date.now();
      if (this.dedupCache.has(msg.messageId)) return;
      this.dedupCache.set(msg.messageId, now);
      for (const [k, v] of this.dedupCache) {
        if (now - v > DEDUP_TTL_MS) this.dedupCache.delete(k);
      }
    }
  ...
  handleChatFinal(runId: string, payload: Record<string, unknown>): void {
    ...
    const text = typeof payload['text'] === 'string' && payload['text']
      ...
      : typeof payload['message'] === 'string' && payload['message']
        ? payload['message'] : '';
    if (text) void this.ilink.sendMessage(userId, text);
  }
